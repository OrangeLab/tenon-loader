...
class VueLoaderPlugin {
  ...
  apply(compiler: Compiler) {
    ...
    for (const rawRule of rules) {
      ...
      vueRules = match(rawRule, `foo.${type}`)
      if (!vueRules.length) {
        vueRules = match(rawRule, `foo.${type}.html`)
      }
      if (vueRules.length > 0) {
        if (rawRule.oneOf) {
          throw new Error(
            `[VueLoaderPlugin Error] tenon-loader currently does not support ${type} rules with oneOf.`
          )
        }
        rawVueRule = rawRule
        break
    ...
    if (!vueRules.length) {
      throw new Error(
        `[VueLoaderPlugin Error] No matching rule for .${type} files found.\n` +
          `Make sure there is at least one root-level rule that matches .${type} or .${type}.html files.`
      )
    }
    ...
    if (vueLoaderUseIndex < 0) {
      throw new Error(
        `[VueLoaderPlugin Error] No matching use for tenon-loader is found.\n` +
          `Make sure the rule matching .${type} files include tenon-loader in its use.`
      )
    }
